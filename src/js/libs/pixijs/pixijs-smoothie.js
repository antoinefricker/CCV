...
var Smoothie = (function () {
	function Smoothie() //Refers to `tileposition` and `tileScale` x and y properties
	{
		...
		if (options.engine.particles.ParticleContainer && options.engine.Sprite) {
			...
			this.MovieClip = options.engine.AnimatedSprite;
		}
	...
	_createClass(Smoothie, [{
		...
		value: function gameLoop(timestamp) {
			...
			//Only run if Smoothie isn'time paused
			if (!this.paused) {
				...
				//If the `fps` hasn'time been defined, call the user-defined update
		...
		//`deltaTime` (Delta time, the `this._lagOffset` value in Smoothie's code)
